...
function toggleSidebar() {
    ...
    if (sidebar) {
      ...
      if (appMain) {
        if (isCollapsed) {
          appMain.style.marginLeft = '80px';
          appMain.style.width = 'calc(100% - 80px)';
        } else {
          appMain.style.marginLeft = '320px';
          appMain.style.width = 'calc(100% - 320px)';
        }
      }
  ...
  function initializeSidebarState() {
    ...
    if (sidebar) {
      if (isCollapsed) {
        ...
        // Update layout untuk state collapsed
        const appMain = document.querySelector('.app-main');
        const topbar = document.querySelector('.topbar');
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    class UserManagement {
        ...
        performSearch() {
            ...
            const role = this.roleFilter ? this.roleFilter.value : '';
            ...
            // Gunakan route dari Laravel blade syntax (ini akan diganti server-side)
            const searchRoute = '/admin/kelola-user/search';
        ...
        handleDeleteClick(e, button) {
            ...
            this.confirmDelete(userName, userEmail, deleteUrl, button);
        }
  
        confirmDelete(userName, userEmail, deleteUrl, button) {
            Swal.fire({
                title: 'Hapus User?',
                html: `
                    <div class="text-left">
                ...
                if (result.isConfirmed) {
                    this.deleteUser(userName, deleteUrl, button);
                }
            });
        }
  
        deleteUser(userName, deleteUrl, button) {
