<x-app-layout>
    <x-slot name="title">SIPMATSAR - Rekap Stok Material</x-slot>

    @push('styles')
    <link rel="stylesheet" href="{{ asset('css/stok-material.css') }}">
    @endpush

    <!-- Notification Container -->
    <div id="notificationContainer" class="notification-container"></div>

    <div class="sipmatsar-stok-material">
        <!-- Tab Navigation -->
        <div class="tab-navigation-wrapper">
            <div class="tab-navigation">
                <button class="tab-button active" data-tab="rekap-stok">
                    <div class="tab-icon">
                        <ion-icon name="bar-chart-outline"></ion-icon>
                    </div>
                    <div class="tab-text">
                        <div class="tab-title">Rekap Stok</div>
                        <div class="tab-subtitle">Data lengkap stok material</div>
                    </div>
                </button>
                
                <button class="tab-button" data-tab="belum-ago">
                    <div class="tab-icon">
                        <ion-icon name="time-outline"></ion-icon>
                    </div>
                    <div class="tab-text">
                        <div class="tab-title">Belum AGO</div>
                        <div class="tab-subtitle">Monitoring approval</div>
                    </div>
                </button>
                
                <button class="tab-button" data-tab="normalisasi">
                    <div class="tab-icon">
                        <ion-icon name="barcode-outline"></ion-icon>
                    </div>
                    <div class="tab-text">
                        <div class="tab-title">Normalisasi</div>
                        <div class="tab-subtitle">Kode standar material</div>
                    </div>
                </button>
            </div>
        </div>

        <!-- Main Content -->
        <div class="main-content">
            <div class="tab-content">
                <!-- TAB 1: REKAP STOK -->
                <div id="rekap-stok" class="tab-pane active">
                    <div class="content-card">
                        <!-- Action Bar -->
                        <div class="action-bar">
                            <div class="action-left">
                                <h3 class="section-title">
                                    <ion-icon name="list-outline"></ion-icon>
                                    Data Rekap Stok Material
                                </h3>
                                <p class="section-subtitle">Management stok material PLN</p>
                            </div>
                            <div class="action-right">
                                <div class="action-group">
                                    <div class="search-box">
                                        <ion-icon name="search-outline" class="search-icon"></ion-icon>
                                        <input type="text" placeholder="Cari material..." class="search-input" id="searchRekap">
                                        <button class="search-clear" id="clearSearchRekap">
                                            <ion-icon name="close-outline"></ion-icon>
                                        </button>
                                    </div>
                                    <div class="export-buttons">
                                        {{-- PERBAIKAN: Tambahkan prefix admin. --}}
                                        <a href="{{ route('admin.rekap-stok.export.pdf') }}" class="btn btn-pdf" id="exportPdfRekap">
                                            <ion-icon name="document-outline"></ion-icon>
                                            PDF
                                        </a>
                                        <a href="{{ route('admin.rekap-stok.export.excel') }}" class="btn btn-excel" id="exportExcelRekap">
                                            <ion-icon name="download-outline"></ion-icon>
                                            Excel
                                        </a>
                                    </div>
                                    <!-- HAPUS TOMBOL TAMBAH STOK KARENA INI HANYA VIEW -->
                                </div>
                            </div>
                        </div>

                        <!-- Table Container -->
                        <div class="table-wrapper">
                            <div class="table-container">
                                <table class="modern-table" id="rekapTable">
                                    <thead>
                                        <tr>
                                            <th class="text-center" rowspan="2">NO</th>
                                            <th rowspan="2">MATERIAL</th>
                                            <th class="text-center" rowspan="2">STOK AWAL</th>
                                            <th class="text-center" rowspan="2">PENERIMAAN</th>
                                            <th class="text-center" colspan="3">PENGELUARAN</th>
                                            <th class="text-center" colspan="2">PLN</th>
                                            <th class="text-center" rowspan="2">STOK AKHIR</th>
                                            <th class="text-center" rowspan="2">MMS</th>
                                            <th class="text-center" rowspan="2">SELISIH</th>
                                            <th class="text-center" rowspan="2">KETERANGAN</th>
                                            <th class="text-center" colspan="4">STOK LEBIH DI LUAR AGO</th>
                                            <th class="text-center" rowspan="2">AKSI</th>
                                        </tr>
                                        <tr>
                                            <th class="text-center">YANBUNG</th>
                                            <th class="text-center">P2TL</th>
                                            <th class="text-center">GANGGUAN</th>
                                            <th class="text-center">IN TRANSIT (PENERIMAAN)</th>
                                            <th class="text-center">IN TRANSIT (PENGELUARAN)</th>
                                            <th class="text-center">KOEF SELISIH</th>
                                            <th class="text-center">SISA REAL</th>
                                            <th class="text-center">TANGGAL</th>
                                            <th class="text-center">WAKTU</th>
                                        </tr>
                                    </thead>
                                    <tbody id="stock-table-body">
                                        @if(isset($materials) && $materials->count() > 0)
                                            @foreach($materials as $index => $material)
                                            <tr>
                                                <td class="text-center">{{ $index + 1 }}</td>
                                                <td>
                                                    <div class="material-info">
                                                        <strong>{{ $material->nama_material ?? 'N/A' }}</strong>
                                                        @if(isset($material->kode_material))
                                                            <small class="text-muted">Kode: {{ $material->kode_material }}</small>
                                                        @endif
                                                    </div>
                                                </td>
                                                <td class="text-center">{{ $material->stok ?? 0 }}</td>
                                                <td class="text-center">{{ $material->total_penerimaan ?? 0 }}</td>
                                                <td class="text-center">{{ $material->pengeluaran_yanbung ?? 0 }}</td>
                                                <td class="text-center">{{ $material->pengeluaran_p2tl ?? 0 }}</td>
                                                <td class="text-center">{{ $material->pengeluaran_gangguan ?? 0 }}</td>
                                                <td class="text-center">{{ $material->pln_in_transit_penerimaan ?? 0 }}</td>
                                                <td class="text-center">{{ $material->pln_in_transit_pengeluaran ?? 0 }}</td>
                                                <td class="text-center">
                                                    <span class="stok-akhir {{ ($material->stok_aktual ?? 0) < ($material->mms ?? 0) ? 'stok-rendah' : 'stok-aman' }}">
                                                        {{ $material->stok_aktual ?? $material->stok ?? 0 }}
                                                    </span>
                                                </td>
                                                <td class="text-center">{{ $material->mms ?? 0 }}</td>
                                                <td class="text-center">
                                                    @php
                                                        $selisih = ($material->stok_aktual ?? $material->stok ?? 0) - ($material->mms ?? 0);
                                                    @endphp
                                                    <span class="selisih {{ $selisih < 0 ? 'selisih-negatif' : 'selisih-positif' }}">
                                                        {{ $selisih }}
                                                    </span>
                                                </td>
                                                <td class="text-center">
                                                    @if(($material->stok_aktual ?? $material->stok ?? 0) < ($material->mms ?? 0))
                                                        <span class="status-badge status-warning">Stok Rendah</span>
                                                    @else
                                                        <span class="status-badge status-success">Aman</span>
                                                    @endif
                                                </td>
                                                <td class="text-center">{{ $material->koef_selisih ?? 0 }}</td>
                                                <td class="text-center">{{ $material->sisa_real ?? 0 }}</td>
                                                <td class="text-center">{{ $material->tanggal_stok_lebih ?? '-' }}</td>
                                                <td class="text-center">{{ $material->waktu_stok_lebih ?? '-' }}</td>
                                                <td class="text-center">
                                                    <div class="action-buttons">
                                                        {{-- PERBAIKAN: Tambahkan prefix admin. --}}
                                                        <a href="{{ route('admin.rekap-stok.detail', $material->id) }}" 
                                                           class="btn-action btn-view" title="Detail">
                                                            <ion-icon name="eye-outline"></ion-icon>
                                                        </a>
                                                    </div>
                                                </td>
                                            </tr>
                                            @endforeach
                                        @else
                                            <tr>
                                                <td colspan="19" class="text-center empty-state">
                                                    <div class="empty-content">
                                                        <ion-icon name="cube-outline" class="empty-icon"></ion-icon>
                                                        <span class="empty-text">Tidak ada data stok material</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        @endif
                                    </tbody>
                                </table>
                            </div>
                        </div>

                        <!-- Table Footer -->
                        <div class="table-footer">
                            <div class="footer-info">
                                <div class="pagination-info">
                                    Menampilkan <strong id="display-count">{{ $materials->count() ?? 0 }}</strong> dari <strong id="total-count">{{ $materials->count() ?? 0 }}</strong> data
                                </div>
                            </div>
                            <div class="footer-actions">
                                <div class="last-update">
                                    <ion-icon name="time-outline"></ion-icon>
                                    Terakhir update: <span id="lastUpdateTime">{{ now()->format('d/m/Y H:i') }}</span>
                                </div>
                                <button class="btn btn-refresh" id="refreshData">
                                    <ion-icon name="refresh-outline"></ion-icon>
                                    Refresh Data
                                </button>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- TAB 2: BELUM AGO -->
                <div id="belum-ago" class="tab-pane">
                    <div class="content-card">
                        <div class="action-bar">
                            <div class="action-left">
                                <h3 class="section-title">
                                    <ion-icon name="time-outline"></ion-icon>
                                    Monitoring Belum AGO
                                </h3>
                                <p class="section-subtitle">Data material yang sedang dalam proses approval</p>
                            </div>
                            <div class="action-right">
                                <div class="action-group">
                                    <div class="search-box">
                                        <ion-icon name="search-outline" class="search-icon"></ion-icon>
                                        <input type="text" placeholder="Cari material..." class="search-input" id="searchAgo">
                                    </div>
                                    <div class="export-buttons">
                                        <button class="btn btn-pdf" id="exportPdfAgo">
                                            <ion-icon name="document-outline"></ion-icon>
                                            PDF
                                        </button>
                                        <button class="btn btn-excel" id="exportExcelAgo">
                                            <ion-icon name="download-outline"></ion-icon>
                                            Excel
                                        </button>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div class="table-wrapper">
                            <div class="table-container">
                                <table class="modern-table" id="agoTable">
                                    <thead>
                                        <tr>
                                            <th class="text-center">NO</th>
                                            <th>MATERIAL</th>
                                            <th class="text-center">SEDANG DIPAKAI</th>
                                            <th class="text-center">SELISIH MMS</th>
                                        </tr>
                                    </thead>
                                    <tbody id="ago-table-body">
                                        <tr>
                                            <td colspan="4" class="text-center empty-state">
                                                <div class="empty-content">
                                                    <ion-icon name="time-outline" class="empty-icon"></ion-icon>
                                                    <span class="empty-text">Tidak ada data approval</span>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- TAB 3: NORMALISASI -->
                <div id="normalisasi" class="tab-pane">
                    <div class="content-card">
                        <div class="action-bar">
                            <div class="action-left">
                                <h3 class="section-title">
                                    <ion-icon name="barcode-outline"></ion-icon>
                                    Kode Normalisasi Material
                                </h3>
                                <p class="section-subtitle">Management kode standar material PLN</p>
                            </div>
                            <div class="action-right">
                                <div class="action-group">
                                    <div class="search-box">
                                        <ion-icon name="search-outline" class="search-icon"></ion-icon>
                                        <input type="text" placeholder="Cari material atau kode..." class="search-input" id="searchNormalisasi">
                                    </div>
                                    <div class="export-buttons">
                                        <button class="btn btn-pdf" id="exportPdfNormalisasi">
                                            <ion-icon name="document-outline"></ion-icon>
                                            PDF
                                        </button>
                                        <button class="btn btn-excel" id="exportExcelNormalisasi">
                                            <ion-icon name="download-outline"></ion-icon>
                                            Excel
                                        </button>
                                    </div>
                                    {{-- PERBAIKAN: Tambahkan prefix admin. --}}
                                    <a href="{{ route('admin.normalisasi.create') }}" class="btn btn-primary" id="tambahNormalisasiBtn">
                                        <ion-icon name="add-outline"></ion-icon>
                                        Tambah Kode
                                    </a>
                                </div>
                            </div>
                        </div>

                        <div class="table-wrapper">
                            <div class="table-container">
                                <table class="modern-table" id="normalisasiTable">
                                    <thead>
                                        <tr>
                                            <th class="text-center">NO</th>
                                            <th>MATERIAL</th>
                                            <th class="text-center">KODE NORMALISASI</th>
                                            <th class="text-center">AKSI</th>
                                        </tr>
                                    </thead>
                                    <tbody id="normalisasi-table-body">
                                        <tr>
                                            <td colspan="4" class="text-center empty-state">
                                                <div class="empty-content">
                                                    <ion-icon name="barcode-outline" class="empty-icon"></ion-icon>
                                                    <span class="empty-text">Tidak ada data normalisasi</span>
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    @push('scripts')
    <script src="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/js/all.min.js"></script>
    <script type="module" src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.esm.js"></script>
    <script nomodule src="https://unpkg.com/ionicons@7.1.0/dist/ionicons/ionicons.js"></script>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Tab Navigation
            const tabButtons = document.querySelectorAll('.tab-button');
            const tabPanes = document.querySelectorAll('.tab-pane');

            tabButtons.forEach(button => {
                button.addEventListener('click', function() {
                    const targetTab = this.getAttribute('data-tab');
                    
                    // Update active tab button
                    tabButtons.forEach(btn => btn.classList.remove('active'));
                    this.classList.add('active');
                    
                    // Update active tab pane
                    tabPanes.forEach(pane => {
                        pane.classList.remove('active');
                        if (pane.id === targetTab) {
                            pane.classList.add('active');
                        }
                    });
                });
            });

            // Search functionality
            const searchInputs = document.querySelectorAll('.search-input');
            searchInputs.forEach(input => {
                input.addEventListener('input', function() {
                    const searchValue = this.value.toLowerCase();
                    const tableId = this.id.replace('search', '').toLowerCase();
                    const table = document.getElementById(tableId + 'Table');
                    
                    if (table) {
                        const rows = table.querySelectorAll('tbody tr');
                        let visibleCount = 0;
                        
                        rows.forEach(row => {
                            const text = row.textContent.toLowerCase();
                            const isVisible = text.includes(searchValue);
                            row.style.display = isVisible ? '' : 'none';
                            
                            if (isVisible) visibleCount++;
                        });
                        
                        // Update display count
                        document.getElementById('display-count').textContent = visibleCount;
                    }
                });
            });

            // Clear search
            const clearSearchBtn = document.getElementById('clearSearchRekap');
            if (clearSearchBtn) {
                clearSearchBtn.addEventListener('click', function() {
                    document.getElementById('searchRekap').value = '';
                    const rows = document.querySelectorAll('#rekapTable tbody tr');
                    rows.forEach(row => row.style.display = '');
                    document.getElementById('display-count').textContent = {{ $materials->count() ?? 0 }};
                });
            }

            // Refresh data
            const refreshBtn = document.getElementById('refreshData');
            if (refreshBtn) {
                refreshBtn.addEventListener('click', function() {
                    this.classList.add('refreshing');
                    setTimeout(() => {
                        location.reload();
                    }, 1000);
                });
            }

            // AJAX untuk mendapatkan data
            function loadRekapData(search = '') {
                fetch(`/admin/rekap-stok/api/data?search=${encodeURIComponent(search)}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            updateTable(data.data);
                            document.getElementById('display-count').textContent = data.data.length;
                            document.getElementById('total-count').textContent = data.data.length;
                            document.getElementById('lastUpdateTime').textContent = data.last_update;
                        }
                    })
                    .catch(error => console.error('Error loading data:', error));
            }

            function updateTable(data) {
                const tbody = document.getElementById('stock-table-body');
                if (!tbody) return;

                if (data.length === 0) {
                    tbody.innerHTML = `
                        <tr>
                            <td colspan="19" class="text-center empty-state">
                                <div class="empty-content">
                                    <ion-icon name="cube-outline" class="empty-icon"></ion-icon>
                                    <span class="empty-text">Tidak ada data stok material</span>
                                </div>
                            </td>
                        </tr>
                    `;
                    return;
                }

                let html = '';
                data.forEach((item, index) => {
                    const selisih = (item.stok_aktual || item.stok || 0) - (item.mms || 0);
                    html += `
                    <tr>
                        <td class="text-center">${index + 1}</td>
                        <td>
                            <div class="material-info">
                                <strong>${item.nama_material || 'N/A'}</strong>
                                ${item.kode_material ? `<small class="text-muted">Kode: ${item.kode_material}</small>` : ''}
                            </div>
                        </td>
                        <td class="text-center">${item.stok || 0}</td>
                        <td class="text-center">${item.total_penerimaan || 0}</td>
                        <td class="text-center">${item.pengeluaran_yanbung || 0}</td>
                        <td class="text-center">${item.pengeluaran_p2tl || 0}</td>
                        <td class="text-center">${item.pengeluaran_gangguan || 0}</td>
                        <td class="text-center">${item.pln_in_transit_penerimaan || 0}</td>
                        <td class="text-center">${item.pln_in_transit_pengeluaran || 0}</td>
                        <td class="text-center">
                            <span class="stok-akhir ${(item.stok_aktual || item.stok || 0) < (item.mms || 0) ? 'stok-rendah' : 'stok-aman'}">
                                ${item.stok_aktual || item.stok || 0}
                            </span>
                        </td>
                        <td class="text-center">${item.mms || 0}</td>
                        <td class="text-center">
                            <span class="selisih ${selisih < 0 ? 'selisih-negatif' : 'selisih-positif'}">
                                ${selisih}
                            </span>
                        </td>
                        <td class="text-center">
                            ${(item.stok_aktual || item.stok || 0) < (item.mms || 0) ? 
                                '<span class="status-badge status-warning">Stok Rendah</span>' : 
                                '<span class="status-badge status-success">Aman</span>'}
                        </td>
                        <td class="text-center">${item.koef_selisih || 0}</td>
                        <td class="text-center">${item.sisa_real || 0}</td>
                        <td class="text-center">${item.tanggal_stok_lebih || '-'}</td>
                        <td class="text-center">${item.waktu_stok_lebih || '-'}</td>
                        <td class="text-center">
                            <div class="action-buttons">
                                <a href="/admin/rekap-stok/detail/${item.id}" 
                                   class="btn-action btn-view" title="Detail">
                                    <ion-icon name="eye-outline"></ion-icon>
                                </a>
                            </div>
                        </td>
                    </tr>
                    `;
                });

                tbody.innerHTML = html;
            }

            // Event listener untuk search real-time
            const searchRekap = document.getElementById('searchRekap');
            if (searchRekap) {
                let searchTimeout;
                searchRekap.addEventListener('input', function() {
                    clearTimeout(searchTimeout);
                    searchTimeout = setTimeout(() => {
                        loadRekapData(this.value);
                    }, 500);
                });
            }
        });
    </script>
    @endpush
</x-app-layout>